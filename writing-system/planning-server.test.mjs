...
test('HTTP draft survives reopening, preserves old snapshots and never completes council',async t=>{
  const f=await setup(t),before=fingerprint(f.root),meeting=council(f.root,'draft test');
  const payload={operation_id:crypto.randomUUID(),base:f.context.base,meeting_id:meeting.id,reviews:meeting.requests.map((r,i)=>({role:r.role,output:i?'':'途中の意見'})),resolution:{execution_mode:'',summary:'',objections:[],pending:[]}};
  const first=await f.post('/api/save-council-draft',payload);assert.equal(first.status,200);const a=await first.json();assert.equal(a.status,'draft');
  assert.equal((await(await f.post('/api/save-council-draft',payload)).json()).id,a.id);
  const b=await(await f.post('/api/save-council-draft',{...payload,operation_id:crypto.randomUUID(),resolution:{...payload.resolution,summary:'仮の結論'}})).json();assert.notEqual(a.id,b.id);
  const init=await(await fetch(f.url+'/api/init')).json(),history=await(await fetch(f.url+'/api/history',{headers:{'X-Planning-Token':init.token}})).json();
  assert.equal(history.drafts.length,2);assert.equal(history.drafts.find(d=>d.id===a.id).payload.resolution.summary,'');assert.equal(history.councils[0].status,'awaiting_reviews');assert.ok(history.councils[0].reviews.every(r=>r.output===null));assert.equal(fingerprint(f.root),before);assert.ok(!fs.existsSync(path.join(f.root,'system/inbox')));
  assert.equal((await f.post('/api/finish-council',payload)).status,400);
  const file=path.join(f.root,'system/council-drafts',a.id+'.json');const changed=JSON.parse(fs.readFileSync(file,'utf8'));changed.payload.reviews[0].output='tampered';fs.writeFileSync(file,JSON.stringify(changed));
  const checked=await(await fetch(f.url+'/api/history',{headers:{'X-Planning-Token':init.token}})).json();assert.equal(checked.drafts.length,1);assert.ok(checked.issues.some(i=>i.kind==='council-drafts'));assert.ok(!JSON.stringify(checked).includes('tampered'));
});
