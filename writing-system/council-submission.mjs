...
export function councilSubmission(root,input,{draft=false}={}){
  ...
  if(!['awaiting_reviews','reviewed_proposal'].includes(meeting.status)||!Array.isArray(meeting.requests)||meeting.requests.length!==roles.length||new Set(meeting.requests.map(r=>r.role)).size!==roles.length||new Set(meeting.requests.map(r=>r.request_id)).size!==roles.length)throw Error('会議記録を確認してください。');
  ...
  if(!Array.isArray(input.reviews)||input.reviews.length!==roles.length||new Set(input.reviews.map(r=>r?.role)).size!==roles.length)throw Error('会議の全担当の回答を入力してください。');
  const replies=meeting.requests.map(req=>{
    const review=input.reviews.find(r=>r?.role===req.role);
    if(typeof review?.output!=='string'||!draft&&!review.output.trim())throw Error('会議の全担当の回答を入力してください。');
  ...
  if(!r||!(draft?['','conversation_single_assistant','external_workers']:['conversation_single_assistant','external_workers']).includes(r.execution_mode)||typeof r.summary!=='string'||!draft&&!r.summary.trim()||!Array.isArray(r.objections)||!Array.isArray(r.pending)||[...r.objections,...r.pending].some(x=>typeof x!=='string'||!x.trim()))throw Error('実行方式・結論・異論・保留の入力を確認してください。');
  const resolution={execution_mode:r.execution_mode,summary:r.summary,objections:r.objections,pending:r.pending};
  if(meeting.status==='reviewed_proposal'&&(draft||meeting.result_digest!==JSON.stringify({replies,resolution})))throw Error('保存済みの会議を別の内容で上書きできません。');
